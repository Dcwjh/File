...
def showCluster(centroidList, clusterDict):
    ...
    展示聚类结果
    '''
    colorMark =    ['or', 'ob', 'og', 'ok', 'oy', 'oc', 'om','or']  # 不同簇类的标记 'or' --> 'o'代表圆，'r'代表red，'b':blue
    centroidMark = ['dr', 'db', 'dg', 'dk', 'dy', 'oc', 'dm','dr']  # 质心标记 同上'd'代表棱形
    for key in clusterDict.keys():
...
if __name__ == '__main__':
    number_k = int(input("输入聚类个数："))
    inFile = "dataSet.csv"  # 数据集文件
    ...
    # print('簇类')
    # for key in clusterDict.keys():
    #     print(key, ' --> ', clusterDict[key]) # 输出每个点的坐标
